...
class DirectProvider:
    ...
    def _call_gemini(self, prompt: str, system_instruction: Optional[str] = None, temperature: float = 0.3) -> str:
        import google.generativeai as genai
        model = self.client
    ...
    def _call_openai(self, prompt: str, system_instruction: Optional[str] = None, temperature: float = 0.3) -> str:
        messages = []
        if system_instruction:
    ...
    def generate_response(self, prompt: str, system_instruction: Optional[str] = None, temperature: float = 0.3) -> str:
        ...
        return self._call_openai(prompt, system_instruction, temperature)
...
class AIProviderService:
    ...
    def _init_providers(self):
        configs = [
            ...
            ("groq", settings.GROQ_MODEL, settings.GROQ_API_KEY, "https://api.groq.com/openai/v1"),
        ...
        for name, model, api_key, base_url in configs:
            if api_key:
                p = DirectProvider(name, model, api_key, base_url)
                if p.is_available():
